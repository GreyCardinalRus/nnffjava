...
public class mlp1 extends Applet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 3573348320821142839L;
	...
	// I can't use classes or structures - the program doesn't seem to work
	// properly
	// What a nuisance!
	...
	// hl_error contains the error for the node i.e. desired output - actual
	// output
	...
	public void paint(Graphics g) {
		...
		// Draw the + and - symbols for increasing/decreasing the number of
		// training patterns
		...
		// Draw the output activity from the output layer
		g.setColor(Color.black);
		for (i = 0; i < OUTMAX; i++) {
			g.drawRect(510, 44 + 25 * i, 90, 12);
	...
	// Draw a filled rectangle with a given x, y, width, height. The parameter
	// 'val' specifies
	// what colour to fill it with
	public void filledRect(Graphics g, int x, int y, int width, int height,
			double val) {
		g.setColor(Color.black);
		g.drawRect(x, y, width, height);
		setColour(g, val);
		g.fillRect(x + 1, y + 1, width - 2, height - 2); // Careful not to draw
															// over black
															// outline
	}
	...
	public void setColour(Graphics g, double v) {
		int temp = (int) (v * 10); // Converts to integer so that I can use
									// 'switch'
		switch (temp) {
		case 0:
	...
	public boolean action(Event e, Object arg) {
		if (e.target instanceof TextField) // If clicked on text field, enter
											// value
		{
	...
	public boolean mouseDown(Event e, int x, int y) {
		...
		// Has the mouse been clicked over the output part of a training
		// pattern?
		for (i = 0; i < numTrainingPat; i++)
			for (j = 0; j < OUTMAX; j++) {
				int xx, yy;
		...
		// Has the mouse been clicked on the + or - to change the number of
		// training patterns?
		if (x > 100 && x < 115 && y > 35 && y < 50
				&& numTrainingPat < MAXTRAINPAT) {
			numTrainingPat++;
			redraw = true;
		}
	...
	public double increase(double v) {
		int temp = (int) (v * 10); // 'switch' doesn't like double, so recast as
									// int
		switch (temp) {
		case 0:
			v = 0.3f;
	...
	public void calculate_output_layer_errors() {
		int i;
		for (i = 0; i < OUTMAX; i++)
			ol_error[i] = (desired[trainingPat][i] - ol_a[i]) * ol_a[i]
					* (1 - ol_a[i]);
	}
	...
	public void weight_change() {
		...
		for (i = 0; i < OUTMAX; i++) {
			for (j = 0; j < HIDDMAX; j++) {
				ol_change[j][i] = GAIN * ol_error[i] * hl_a[j] + ACL
						* ol_change[j][i];
		...
		for (i = 0; i < HIDDMAX; i++) {
			for (j = 0; j < INPMAX; j++) {
				hl_change[j][i] = GAIN * hl_error[i]
						* trainingPattern[trainingPat][j] + ACL
						* hl_change[j][i];
